...
public class WriteLog : IDisposable
{
    ...
    #region void CommonLog (string msg)
    private static void CommonLog (string msg)
    {
        CommonLog(String.Empty, msg);
    }
    #endregion void CommonLog (string msg)

    #region void CommonLog (string txt, string msg)
    private static void CommonLog (string txt, string msg)
    {
        WriteLog wl = new WriteLog();
        if (txt.Length > 0)
            wl.LogMsg(String.Format("{0} = {1}", txt, msg));
        else
            wl.LogMsg(msg);
        wl.Close();
        wl = null;
    }
    #endregion void CommonLog (string txt, string msg)

    #region void CommonLog<T> (T msg)
    private static void CommonLog<T> (T msg)
    {
        CommonLog(String.Empty, msg);
    }
    #endregion void CommonLog<T> (T msg)

    #region void CommonLog<T> (string txt, T msg)
    ...
    #endregion void CommonLog<T> (string txt, T msg)

    #region void CommonLog1 (string fileName, string msg)
    private static void CommonLog1 (string fileName, string msg)
    {
        CommonLog1(fileName, String.Empty, msg);
    }
    #endregion void CommonLog1 (string fileName, string msg)

    #region void CommonLog1 (string fileName, string txt, string msg)
    private static void CommonLog1 (string fileName, string txt, string msg)
    {
        WriteLog wl = new WriteLog();
        wl.OutputFilename = fileName;
        if (txt.Length > 0)
            wl.LogMsg(String.Format("{0} = {1}", txt, msg));
        else
            wl.LogMsg(msg);
    ...
    #endregion void CommonLog1 (string fileName, string txt, string msg)
